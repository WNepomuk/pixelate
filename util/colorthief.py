...
class ColorThief(object):
    ...
    def __init__(self, file):
        ...
        :param file: A filename (string) or a file object or a PIL Image. The file object
                     must implement `read()`, `seek()`, and `tell()` methods,
                     and be opened in binary mode.
        """

        if type(file) == Image.Image or type(file) == JpegImageFile or type(file) == PngImageFile or type(file) == WebPImageFile:
            self.image = file
        else:
